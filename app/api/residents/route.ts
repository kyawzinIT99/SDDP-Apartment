...
export async function POST(request: Request) {
  ...
  const record = {
    ...
    passportCiphertext, passportLast4: passport.slice(-4), roomNumber: clean(input.roomNumber, 20),
    checkInDate: clean(input.checkInDate, 20), checkOutDate: clean(input.checkOutDate, 20), now,
  };
