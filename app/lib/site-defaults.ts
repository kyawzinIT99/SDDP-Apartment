...
export function officialLineId(value: string) {
  const id = value.trim().replace(/^@/, "");
  return id || defaultSiteSettings.lineId;
}

export function lineChatUrl(value: string) {
  return `https://line.me/ti/p/~${officialLineId(value)}`;
}
