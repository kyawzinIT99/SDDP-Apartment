  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main">
      ...
      {tab === "content" && <form className="editor-grid" onSubmit={save}>
        ...
        <section className="editor-card"><div className="card-head"><div><span>BANK TRANSFER</span><h2>Deposit account</h2><p>Shown in the automated inquiry email. Guests transfer 50% then send the slip to Line.</p></div></div><label>Bank name<input value={settings.bankName} onChange={(e) => field("bankName", e.target.value)} placeholder="e.g. Kasikornbank" /></label><label>Account name<input value={settings.bankAccountName} onChange={(e) => field("bankAccountName", e.target.value)} /></label><label>Account number<input value={settings.bankAccountNumber} onChange={(e) => field("bankAccountNumber", e.target.value)} /></label><label>PromptPay<input value={settings.bankPromptPay} onChange={(e) => field("bankPromptPay", e.target.value)} placeholder="Optional" /></label></section>
      ...
      {tab === "automation" && <div className="automation-grid">
        ...
        <section className="editor-card flow-card">
          ...
          <p>Start with inquiries and occupancy. Print the original SDDP invoice from the Invoices tab. The Render disk keeps those records across deploys. Staff alerts use a new n8n workflow named SDDP Inquiry Alert on the existing Hostinger instance. Guests receive a professional deposit email (50% to reserve, non-refundable if they do not arrive, bank details, send the slip to Line). BCC and PDF workflows are not used. Passport data stays encrypted and is never sent in notifications.</p>
        </section>
      </div>}
    </section>
  </main>;
