...
test("keeps the public facts and automation endpoints in source", async () => {
  ...
  assert.match(occupancy, /SELECT room_number, full_name, phone FROM residents/);
  assert.match(occupancy, /status != 'checked_out'/);
  assert.match(occupancy, /catalogRoomFromValue/);
  assert.doesNotMatch(occupancy, /passport|email/);
