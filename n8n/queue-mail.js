const list = items[0].json.inquiries || [];
...
function build(body) {
  const lineId = String(body.lineId || "SDDP.Apartment").replace(/^@/, "");
  const bankName = body.bankName || "";
  const bankAccountName = body.bankAccountName || "";
  const bankAccountNumber = body.bankAccountNumber || "";
  const bankPromptPay = body.bankPromptPay || "";
  const staffEmail = body.alertEmail || body.staffEmail || "sddpapartment@gmail.com";
  const phone = body.phonePrimary || "094-293-5296";
  const deposit = body.monthlyDeposit || "50% of first month";
  const email = String(body.email || body.guestEmail || "").trim();
...
<body style="margin:0;background:#f3efe6;font-family:'Segoe UI',Tahoma,Arial,sans-serif;color:#1b1c1d">
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:18px;overflow:hidden;border:1px solid #e7e1d5">
        <tr><td style="background:#1b1c1d;padding:22px 28px">
          ...
          <div style="font-size:22px;color:#fff;font-weight:700;margin-top:6px">Your inquiry and deposit details</div>
  ...
  <div style="max-width:560px;margin:auto;background:#fff;border-radius:16px;padding:24px;border:1px solid #e7e1d5">
    ...
    <p><strong>Email:</strong> ${esc(email || "—")}<br><strong>Phone:</strong> ${esc(body.phone || "—")}<br><strong>Stay:</strong> ${esc(stayType)}<br><strong>Arrival:</strong> ${esc(arrivalDate)}</p>
